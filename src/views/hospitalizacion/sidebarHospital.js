    ...
    view: () => {
        ...
        if (_data.length !== 0) {
            return [
                ...
                _data.modulesAccess.hospitalizacion.map(function(_v, _i, _contentData) {
                    return [

                        m(m.route.Link, { href: "/" + _v.href, class: ((SidebarHospital.page == _v.idModulo) ? "active" : "") }, [
                            _v.label
                        ]),
...
const SidebarHospital = {
    page: "",
    setPage: (page) => {
        SidebarHospital.page = page;
    },
    view: () => {
        return [
